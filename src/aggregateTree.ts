...
/*
type AggregateTreeNode = {
    id: number; // TODO: do we need this?
    type: string;
    value: any;
    expander: number;
    frame: number;
...
export class AggregateWorker {
    private config: AggregateWorkerConfig;
    private expander: ((frame: number, row: number) => (any | undefined))[];
    private id: Map<number, AggregateTreeNode>;
    private rowCount: number;
    private aggregateGenerators: AggregateGenerator[];

    private makeNode(type: string, value: any, expander: number, frame: number, rowBegin: number, rowEnd: number): number {
        const id = this.id.size;
        const node = { id, type, value, expander, frame, rowBegin, rowEnd };
        this.id.set(id, node);
        return id;
    }
    ...
    constructor(config: AggregateWorkerConfig) {
        this.aggregateGenerators = validateAggregateWorkerConfig(config);
        this.config = config;
        this.expander = config.expanders.map(i => {
            const column = config.columns[i];
            const data = column.data;
            if (column.stack) {
                return (f, r) => column.frames[data[r]][f];
        ...
        this.id = new Map();
        this.rowCount = config.columns[0].data.length;
        const rootId = this.makeNode('root', 'root', 0, 0, 0, this.rowCount);
        if (rootId !== 0) {
            throw new Error('AggregateWorker root id not 0');
        }
    ...
    private getChildrenImpl(parent: number, expanderIndex: number, frame: number, rowBegin: number, rowEnd: number): number[] {
        if (expanderIndex >= this.expander.length) {
            return [];
        }
        const column = this.config.columns[this.config.expanders[expanderIndex]];
        const expander = this.expander[expanderIndex];
        // Build a map that sorts rows based on the expander value.
        const sameValue = new Map<any | undefined, number[]>(); // expander value => list of rows with that value.
        for (let i = rowBegin; i < rowEnd; i++) {
            const value = expander(frame, i);
        ...
        for (const [value, rows] of sameValue.entries()) {
            ...
            if (value === undefined) {
                ...
                children.push(this.makeNode(column.type, value, expanderIndex, frame + 1, childRowBegin, childRowEnd));
            }
            shuffle.push(...rows);
        }
        if (shuffle.length !== rowEnd - rowBegin) {
        ...
        // Recursively expand rows that have value === undefined, and append them to children list
        if (nextExpanderRange !== null) {
            children.push(...this.getChildrenImpl(parent, expanderIndex + 1, 0, nextExpanderRange.begin, nextExpanderRange.end));
        }
        return children;
    }
    ...
    getChildren(id: number): number[] {
        ...
        return this.getChildrenImpl(id, node.expander, node.frame, node.rowBegin, node.rowEnd);
    }
    ...
    getValue(id: number): { type: string, value: any } {
        const node = this.id.get(id);
        if (node === undefined) {
            throw new Error(`${id} is not a valid node ID`);
        }
